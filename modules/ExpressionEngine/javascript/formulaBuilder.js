...
return true;var url="index.php?"+SU.paramsToUrl({module:"ExpressionEngine",action:"validateRelatedField",tmodule:ModuleBuilder.module,link:t.args[0].name,related:t.args[1].value});var resp=http_fetch_sync(url);var def=YAHOO.lang.JSON.parse(resp.responseText);if(typeof(def)=="string"){throw(t.name+": "+def);}
...
return;}};SUGAR.expressions.validateCurrExpression=function(silent,matchType){try{var varTypeMap={};for(var i=0;i<fieldsArray.length;i++){varTypeMap[fieldsArray[i][0]]=fieldsArray[i][1];}

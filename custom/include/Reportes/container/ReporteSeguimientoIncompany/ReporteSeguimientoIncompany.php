...
class ReporteSeguimientoIncompany extends SP_Reporte {
    protected $parametros;
    public $fecha_inicio;
    public $fecha_final;
    public $archivo="ReporteSeguimientoIncompany";
    ...
    */
    function __construct(& $form){
        global $app_list_strings;
         ...
         $this->reporte="ReporteSeguimientoIncompany";
    ...
    */
    function  setSql() {
        $query="SELECT id,name,
                fechainicio_programa fechainicio,
                fechafinal_programa fechafin,
                numerominimo numero,estado estatus
                FROM ee_programas where deleted=0";
        parent::setSql($query);
    }
   function  groupby() {
        parent::groupby();
...
//        echo $this->sql;
   ...
   function armarFiltros(){
        $filtros=" and  fechainicio_programa between '".$this->form['fecha_inicial']."' and '".$this->form['fecha_final']."'";
     
           $filtros.= " and tipoprograma in ('Incompany')";
     if(count($this->form['estado_programa'])>0)
           $filtros.= " and estado in ('". implode("','",$this->form['estado_programa']) ."')";
    if(count($this->form['modalidad'])>0)
         $filtros.= " and modalidad in ('". implode("','",$this->form['modalidad']) ."')";
    if(count($this->form['programas'])>0)
         $filtros.= " and id in ('". implode("','",$this->form['programas']) ."')";
     return $filtros;
   }
   /*
    *
   ...
   function armarArray($result=array()){
     ...
     while($a = $this->db->fetchByAssoc($result)) {
            ...
            $ingresos=self::totalIngresos($a['id']);
            $ganados=self::totalParticipante($a['id']);
            $facturados=self::totalParticipanteFacturados($a['id']);
            $data['cliente']=self::nombreCliente($a['id']);
//            $data['cliente']=$a['id'];
            $data['programa']=$a['name'];
            $data['fechainicio']=$a['fechainicio'];
            $data['fechafin']=$a['fechafin'];
            $data['estatus']=$app_list_strings['programa_status_dom'][$a['estatus']];
            $data['minimo_pax']=self::numeroPax($a['id']);
            $data['participanteprospecto']=$app_list_strings['sales_stage_dom'][self::totalParticipantesInscritos($a['id'])];
            
            $data['participantereal']=number_format(self::montoOportunidad($a['id']),2);
                       
            $e=self::totalPresupuesto($a['id']);
            $data['egresos']=number_format($e,2);
            $data['ganancia-perdida']=number_format((float)$ganados['valor']-(float)$e,2);
   ...
   */
   function displayParametros(){
      ...
      $this->form['fecha_final']= '2016-12-01';
      ...
      $this->sugar_smarty->assign("selected_programas",$this->programasincompany());
...
return $chart->toPrettyString();
    ...
    function totalParticipantesInscritos($id){
        global $db;
        ...
        $sql="SELECT sales_stage
              ...
              INNER JOIN opportunities o ON o.id = ao.opportunity_id
              inner join detalle_cotizacion d on o.id=d.oportunidad_id
              WHERE o.deleted=0 and d.deleted=0 and a.deleted=0 and ao.deleted=0 and
                    d.idprograma='$id' $filtros";
        $result=$db->query($sql);
        $a=$db->fetchByAssoc($result);
        $total=$a['sales_stage'];
      //  echo $sql;
...
//      if(empty($total))  echo $sql.'<br/><br/>';
    ...
    function nombreCliente($id){
        global $db;
         if(count($this->form['usuarios'])>0)
           $filtros.= " and o.assigned_user_id in ('". implode("','",$this->form['usuarios']) ."')";
        $sql="SELECT a.name AS nombrecliente
              FROM accounts a INNER JOIN accounts_opportunities ao ON a.id = ao.account_id 
              INNER JOIN opportunities o ON o.id = ao.opportunity_id
              inner join detalle_cotizacion d on o.id=d.oportunidad_id
              WHERE o.deleted=0 and d.deleted=0 and a.deleted=0 and ao.deleted=0 and
    ...
    function numeroPax($id){
        global $db;
         if(count($this->form['usuarios'])>0)
           $filtros.= " and o.assigned_user_id in ('". implode("','",$this->form['usuarios']) ."')";
        $sql="SELECT numeroparticipantes
              FROM accounts a INNER JOIN accounts_opportunities ao ON a.id = ao.account_id 
              INNER JOIN opportunities o ON o.id = ao.opportunity_id
              inner join detalle_cotizacion d on o.id=d.oportunidad_id
              WHERE o.deleted=0 and d.deleted=0 and a.deleted=0 and ao.deleted=0 and
    ...
    function totalParticipante($id){
        ...
        $res=array('pagados'=>0,'totales'=>0,'valor'=>0);
         if(count($this->form['usuarios'])>0)
           $filtros.= " and o.assigned_user_id in ('". implode("','",$this->form['usuarios']) ."')";
        $sql="SELECT o.idcobranza idcobranza
              FROM accounts a INNER JOIN accounts_opportunities ao ON a.id = ao.account_id
              INNER JOIN opportunities o ON o.id = ao.opportunity_id
              inner join detalle_cotizacion d on o.id=d.oportunidad_id
              WHERE o.deleted=0 and d.deleted=0 and a.deleted=0 and ao.deleted=0 and
...
//                    $total=(empty($total))? 0 : $total ;
    ...
    function totalParticipanteFacturados($id){
        ...
        $sql="SELECT count(p.id) numero,o.idcobranza
              ...
              INNER JOIN opportunities o ON o.id = ao.opportunity_id
              inner join detalle_cotizacion d on o.id=d.oportunidad_id
              WHERE o.deleted=0 and d.deleted=0 and

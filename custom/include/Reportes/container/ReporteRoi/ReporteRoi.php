...
class ReporteRoi extends SP_Reporte {
    protected $parametros;
    public $fecha_inicio;
    public $fecha_final;
    public $archivo="ReporteRoi";
   ...
   function armarArray($result=array()){
     ...
     while($a = $this->db->fetchByAssoc($result)) {
            ...
            $data['participanteprospecto']=number_format(self::totalParticipantesInscritos($a['id'],"Prospecting"),0);
            $data['participanteseguimienti']=number_format(self::totalParticipantesInscritos($a['id'],"Seguimiento"),0);
            $data['participanteinscritos']=number_format(self::totalParticipantesInscritos($a['id']),0);
            $data['participantereal']=number_format((int)$facturados['totales'],0);
            $data['participanteregistrados']=$ganados['pagados'];
        //    $data['valorregistrado']=number_format(((float)$ingresos['monto']-(float)$ingresos['descuento']),2);//*
            $data['valorreal']=number_format($ganados['valor'],2);            
            $e=self::totalPresupuesto($a['id']);
            $data['egresos']=number_format($e,2);
          //  $data['roi']=self::calculoRoi($data['ingresos'],$data['egresos']);

...
class ReporteClientesRecurrentes extends SP_Reporte {
    protected $parametros;
    public $fecha_inicio;
    public $fecha_final;
    public $archivo="ReporteClientesRecurrentes";
    ...
    */
    function __construct(& $form){
        global $app_list_strings;
         ...
         $this->reporte="ReporteClientesRecurrentes";
    ...
    */
    function  setSql() {
        $query="SELECT COUNT(a.id) AS contador, a.id AS idcliente, a.name AS nombrecliente, dc.programa AS nombreprograma, p.fechainicio_programa AS fechainicio, 
                       p.fechafinal_programa AS fechafin, ocstm.descuento_c AS descuento, motivo_c AS motivodescuento, a.calle_principal AS direccion, 
                       a.ciudad_principal AS ciudad,a.provincia_principal AS provincia, a.phone_office AS telefono, a.phone_alternate AS celular, a.id AS idcuenta, acstm.tipocliente2_c AS tipocliente, o.amountreal AS precioreal,acstm.tipocliente_c AS tipoclientereal
                FROM opportunities o, detalle_cotizacion dc, accounts a, accounts_opportunities ao, ee_programas p, opportunities_cstm ocstm, accounts_cstm acstm
                WHERE o.id=dc.oportunidad_id AND a.id=ao.account_id AND o.id=ao.opportunity_id AND p.id=dc.idprograma AND o.id=ocstm.id_c AND a.id=acstm.id_c
                      AND o.sales_stage = 'Closed Won' AND p.tipoprograma like '%Abierto%'
                      AND o.deleted=0 AND dc.deleted=0 AND a.deleted=0 AND ao.deleted=0 AND p.deleted=0  ";
        parent::setSql($query);
    }
   function  groupby() {
        parent::groupby();
...
//        echo $this->sql;
   ...
   function armarFiltros(){
    ...
    if(count($this->form['tipo_cliente'])>0)
         $filtros.= " and acstm.tipocliente_c = '". $this->form['tipo_cliente'] ."'";
     return $filtros;
   }
   /*
    *
     ...
     global $db;
      ...
      while($a = $this->db->fetchByAssoc($result)) {
         if($a['contador']>1){
            ...
            $data['cliente']=$a['nombrecliente'];
            $empresa = '';
            $cargo = '';
            if(($a['tipoclientereal']=='Natural')||($a['tipoclientereal']=='Extranjero')){
                $sql12 = 'SELECT lugartrabajo, title AS cargo FROM contacts c, accounts_contacts ac, accounts a WHERE a.id=ac.account_id AND c.id=ac.contact_id AND c.deleted=0 AND ac.deleted=0 AND a.id="'.$a['idcuenta'].'"';
                $result12=$db->query($sql12);
                while ($x=$db->fetchByAssoc($result12)){
                    $empresa= $x['lugartrabajo'];
                    $cargo= $x['cargo'];
                }
            }            
            ...
            $query2="SELECT dc.programa AS nombreprograma
                    FROM opportunities o, detalle_cotizacion dc, accounts_opportunities ao
                    WHERE o.id=dc.oportunidad_id AND o.id=ao.opportunity_id 
                          AND ao.account_id = '".$a['idcliente']."' 
                          AND o.deleted=0 AND dc.deleted=0 AND ao.deleted=0  ";
            $result22=$db->query($query2);
            $nombrePrograma = '';
            while ($xx=$db->fetchByAssoc($result22)){
                $nombrePrograma .= $xx['nombreprograma'].'<br/><br/>';
            }
            
            $data['programa']=$nombrePrograma;
            $data['fechainicio']=$a['fechainicio'];
            $data['fechafin']=$a['fechafin'];
            $data['precioreal']=number_format($a['precioreal'],2);
            $data['descuento']=number_format($a['descuento'],2).'%';            
            $data['motivodescuento']=str_replace("^", "", $a['motivodescuento']);
            $data['direccion']=$a['direccion'];
            $data['provincia']=$a['provincia'];
            $data['ciudad']=$a['ciudad'];
            $sql11 = 'SELECT email_address FROM email_addresses ea, email_addr_bean_rel br WHERE ea.id=br.email_address_id AND br.bean_module= "Accounts" AND br.deleted=0 AND ea.deleted=0 AND br.bean_id="'.$a['idcuenta'].'"';
   ...
   */
   function displayParametros(){
      ...
      $this->sugar_smarty->assign("selected_tipo_cliente",$this->lista('tipocliente_list','tipo_cliente'));

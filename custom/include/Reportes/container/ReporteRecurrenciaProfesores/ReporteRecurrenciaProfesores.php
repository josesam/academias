...
class ReporteRecurrenciaProfesores extends SP_Reporte {
    protected $parametros;
    public $fecha_inicio;
    public $fecha_final;
    public $archivo="ReporteRecurrenciaProfesores";
    ...
    */
    function __construct(& $form){
        global $app_list_strings;
         ...
         $this->reporte="ReporteRecurrenciaProfesores";
    ...
    */
    function  setSql() {
        $query="SELECT dp.profesor profesor, p.id idprograma, p.name programa, dp.fecha_inicio fechainicio, dp.fecha_fin fechafin, dp.modulo modulo, p.modalidad modalidad, 
                       dp.horas horas, dp.nivel nivel, dp.lugar ciudad, dp.instalaciones instalaciones, p.tipoprograma AS tipoprograma, dp.idmodulo AS idmodulo
                FROM ee_programas p, detalle_programa dp
                WHERE dp.programa_id=p.id 
   ...
   function armarFiltros(){
        $filtros=" and  dp.fecha_inicio between '".$this->form['fecha_inicial']."' and '".$this->form['fecha_final']."'";
     
//           $filtros.= " and tipoprograma in ('Abierto')";
    ...
    if(count($this->form['profesor'])>0)
         $filtros.= " and dp.idprofesor in ('". implode("','",$this->form['profesor']) ."')";
   ...
   function armarArray($result=array()){
     global $mod_strings,$current_language,$app_list_strings;
      ...
      while($a = $this->db->fetchByAssoc($result)) {
            ...
            $data['tipoprograma']=$a['tipoprograma'];
            ...
            $data['instalaciones']=$a['instalaciones'];
            
            $evaluacion=self::evaluacion($a['idmodulo']);
        
            $data['conocimientoTema']=$evaluacion['conocimientoTema'];
            $data['manejoClase']=$evaluacion['manejoClase'];
            $data['actitud']=$evaluacion['actitud'];
            $data['total']=$evaluacion['total'];
   ...
   */
   function displayParametros(){
      ...
      $this->sugar_smarty->assign("selected_profesor",$this->profesor());
...
return $chart->toPrettyString();
    ...
    function evaluacion($idmodulo){
        global $db;
        $res=array('conocimientoTema'=>0,'manejoClase'=>0,'actitud'=>0,'total'=>0);
        
        $sql="SELECT texto
              FROM ee_evaluacionglobal eg, datos_modulo dm
              WHERE eg.id=dm.oportunidad_id AND eg.idmodulo='$idmodulo' AND dm.nombre='instructor1'";
        $result=$db->query($sql);
        while ($a=$db->fetchByAssoc($result)){
            $conocimientoTema = $a['texto'];
        }
        
        $sql1="SELECT texto
              FROM ee_evaluacionglobal eg, datos_modulo dm
              WHERE eg.id=dm.oportunidad_id AND eg.idmodulo='$idmodulo' AND dm.nombre='instructor2'";
        $result1=$db->query($sql1);
        while ($b=$db->fetchByAssoc($result1)){
            $manejoClase = $b['texto'];
        }
        
        $sql2="SELECT texto
              FROM ee_evaluacionglobal eg, datos_modulo dm
              WHERE eg.id=dm.oportunidad_id AND eg.idmodulo='$idmodulo' AND dm.nombre='instructor3'";
        $result2=$db->query($sql2);
        while ($c=$db->fetchByAssoc($result2)){
            $actitud = $c['texto'];
        }
        
        $sql3="SELECT texto
              FROM ee_evaluacionglobal eg, datos_modulo dm
              WHERE eg.id=dm.oportunidad_id AND eg.idmodulo='$idmodulo' AND dm.nombre='total_instructor'";
        $result3=$db->query($sql3);
        while ($d=$db->fetchByAssoc($result3)){
            $total = $d['texto'];
        }
        
        $res['conocimientoTema']=$conocimientoTema;
        $res['manejoClase']=$manejoClase;
        $res['actitud']=$actitud;
        $res['total']=$total;
        return $res;
    }

...
class ReporteProgramasInteres extends SP_Reporte {
    protected $parametros;
    public $fecha_inicio;
    public $fecha_final;
    public $archivo="ReporteProgramasInteres";
    ...
    */
    function __construct(& $form){
        global $app_list_strings;
         ...
         $this->reporte="ReporteProgramasInteres";
    ...
    */
    function  setSql() {
        $query="SELECT a.name AS cliente, a.cursosinteres AS interes, a.calle_principal AS direccion, a.phone_office AS telefono, 
                       a.phone_alternate AS celular, a.mediocontacto AS medio,a.detallemedio AS detallemedio,a.ciudad_principal AS ciudad,
                       a.provincia_principal AS provincia,
                       acstm.tipocliente_c AS tipocliente, a.id AS idcliente
                FROM accounts a , accounts_cstm acstm
                WHERE a.id=acstm.id_c AND a.deleted=0 AND a.cursosinteres IS NOT NULL AND a.cursosinteres <> '' ";
        parent::setSql($query);
    }
   function  groupby() {
        parent::groupby();
...
//        echo $this->sql;
   ...
   function armarFiltros(){
        $filtros=" and  DATE(a.date_entered) between '".$this->form['fecha_inicial']."' and '".$this->form['fecha_final']."'";
    ...
    if(count($this->form['medio'])>0)
         $filtros.= " and a.mediocontacto in ('". implode("','",$this->form['medio']) ."')";
    
    if(count($this->form['detallemedio'])>0)
         $filtros.= " and a.detallemedio in ('". implode("','",$this->form['detallemedio']) ."')";
     
     if($this->form['programainteres']!='')
         $filtros.= " and a.cursosinteres like '".$this->form['programainteres']."%'";
     
     if(count($this->form['programastodos'])>0)
         $filtros.= " and a.cursosinteres like '%".$this->form['programastodos']."%'";
//         $filtros.= " and a.cursosinteres in ('". implode("','",$this->form['programastodos']) ."')";
...
//          if($a['interes']!=null){
            $data['tipocliente']=$a['tipocliente'];
            $data['cliente']=$a['cliente'];
            $programasInteres = explode(";", $a['interes']);
            $interes='';
            for($i=0;$i<count($programasInteres);$i++){
                $interes .= $programasInteres[$i].'<br/><br/>';
            }
            ...
            $sql99="SELECT email_address 
                  FROM email_addresses ea, email_addr_bean_rel br
                  WHERE ea.id = br.email_address_id AND br.bean_module='Accounts' AND ea.deleted=0 AND br.deleted=0 AND bean_id='".$a['idcliente']."'";
            $result99=$db->query($sql99);
            $mail = '';
            while($b=$db->fetchByAssoc($result99)){
                $mail .= $b['email_address'].'<br/>';
            }
            $data['mail']=$mail;            
            
            
            $data['medio']=$a['medio'].' / '.$a['detallemedio'];
...
//          }
   ...
   */
   function displayParametros(){
      ...
      $this->sugar_smarty->assign('programainteres',$this->form['programainteres']);
      ...
      $this->sugar_smarty->assign("selected_programastodos",$this->programastodos());
      $this->sugar_smarty->assign("selected_medio",$this->medio());
      $this->sugar_smarty->assign("selected_detallemedio",$this->detallemedio());

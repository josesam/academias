...
class ee_EncuestaITCViewModulos extends ViewEdit {

 	function ee_EncuestaITCViewModulos(){
 		parent::ViewEdit();
 	}
	...
	<tr>
        ...
        if(is_array($lista)){
          ...
          foreach ($lista as $key => $row):
        	...
        	$cadena.='<tr>';
			$cadena.='<td><a href="javascript:void(0);" onclick="javascript:selectModulo(\''.$row['modulo'].'\',\''.$row['id'].'\')">'.$row['modulo'].'</a>';//echo CHtml::link($ciudad, 'javascript:' . $js);
                        $cadena.='</td>';
		$cadena.='</tr>';
            endforeach;
          $cadena.="</tbody>";

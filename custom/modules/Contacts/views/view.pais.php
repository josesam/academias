...
class ContactsViewPais extends ViewEdit {

 	function ContactsViewPais(){
 		parent::ViewEdit();
 	}
	...
	<tr>
		...
		$pais = !empty($row['name']) ? $row['name'] : $row['name'];
        	...
        	$cadena.='<tr>';
			$cadena.='<td><a href="javascript:void(0);" onclick="javascript:selectPais('.$js.');">'.$pais.'</a>';//echo CHtml::link($ciudad, 'javascript:' . $js);
                        $cadena.='</td>';
		$cadena.='</tr>';
            endforeach;
        $cadena.="</tbody>";

 	...
 	function display(){
            ...
            if(empty($nombre))
                $sql="Select c.id,c.first_name,c.last_name,c.calle_principal direccion,
                        c.phone_work telefono,a.name,c.cedula,em.email_address email,c.title cargo 
                    ...
                    LEFT JOIN email_addresses em on em.id=h.email_address_id
                      where c.deleted=0 and cstm.participante_c=1  and a.id='$cliente' $filtro LIMIT 0,15 ";
            else
                $sql="Select c.id,c.first_name,c.last_name,c.calle_principal direccion,
                        c.phone_work telefono,a.name,c.cedula,,em.email_address email,c.title cargo 
                      from contacts c inner join accounts_contacts r on c.id=r.contact_id and r.deleted=0
                      ...
                      and c.first_name like '%$nombre%' or c.last_name like '%$nombre%' $filtro";
        ...
        $cadena.="<tbody>";
          foreach ($lista as $key => $row):		
                ...
                $cargo=$row['cargo'];
        	$cadena.='<tr>';
			$cadena.='<td><a href="javascript:void(0);" onclick="javascript:addParticipante(\'OpportunitiesAmbienteTableP\',\'\',\''.$id.'\',\''.$first_name.'\',\''.$last_name.'\',\''.$direccion.'\',\''.$telefono.'\',\''.$cedula.'\',\'database\',\''.$email.'\',\''.$cargo.'\')">'.$first_name.' '.$last_name.'</a>';
                        $cadena.='</td>';
                        $cadena.='<td>'.$cedula.'</td>';

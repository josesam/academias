...
function handleSave($prefix,$redirect=true, $useRequired=false){
	...
	if (isset($GLOBALS['check_notify'])) {
        ...
        if (file_exists('custom/include/clases/varios/ManejoEtapasVenta.php')){
                ...
                $manejo=new ManejoEtapasVenta($_REQUEST['categoria_c']);
                $focus->sales_stage=$manejo->prepareSaveStage($_REQUEST['sales_stage']);
            }
	...
	$focus->save($check_notify);
        ...
        if($focus->sales_stage=='Closed Won'){
            if (!empty($focus->account_id)){
                $sql="Update accounts_cstm set estado_c='Activo' where id_c='".$focus->account_id."'";
                $db = DBManagerFactory::getInstance();
                $result = $db->query($sql);

            }
            $path='custom/include/clases/varios/Cobranza.php';
            if(file_exists($path)){
                include_once $path;
                Cobranza::create($focus);
            }
        }
        if($focus->sales_stage=='Closed Lost'){
            ...
            if(file_exists($path)){
                include_once $path;
                  ...
                  if(!empty($focus->idcobranza)){
                    $message = Cobranza::delete($focus->idcobranza);
                    $GLOBALS['log']->fatal($mensaje);
                    }else{
                        $GLOBALS['log']->fatal("Vacio el id".$focus->idcobranza);
                    }
            }else{
                        $GLOBALS['log']->fatal("Archivo no existe".$focus->idcobranza);
            }
        }else{
                    $GLOBALS['log']->fatal("No esta perdido");
        }
        $path= 'include/SugarCotizacion/SugarCotizacion.php';
           if (file_exists($path)){
              include_once($path);
              $cotizacion=new SugarCotizacion();
              $cotizacion->save($return_id,$focus->module_dir,false,$focus->account_id);
           }
        $path= 'include/SugarParticipantes/SugarParticipantes.php';
           if (file_exists($path)){
              include_once($path);
              $participante=new SugarParticipantes();
              $participante->save($return_id,$focus->module_dir);
           }
           // crea contactos
           $path='custom/include/clases/varios/Contacto.php';
           if (file_exists($path)){
              include_once($path);
              $c=new Contacto($return_id,$focus->sales_stage);
              $c->ejecuta($focus->account_id);
           }
           //Upload Files

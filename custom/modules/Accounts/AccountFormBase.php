...
function handleSave($prefix,$redirect=true, $useRequired=false){
	...
	if(!$focus->ACLAccess('Save')){
        ...
        //Solo cuando este vacio
        
        if (empty($focus->ee_nacionalidad_id_c))
                $focus->ee_nacionalidad_id_c="c8061532-b759-a071-d560-4fded9896adb";
        // Cargar curso de interes
        $datos=$_REQUEST;
        $cadena="";
			...
			$id = str_replace('prod_', '', $key);
                        $cadena.=$datos['cursotxt_' . $id].";";
		}
        $cadena=substr($cadena, 0, strlen($cadena)-1);
        $focus->cursosinteres=$cadena;

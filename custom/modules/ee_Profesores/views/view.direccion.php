...
class ee_ProfesoresViewDireccion extends ViewEdit {

 	function ee_ProfesoresViewDireccion(){
 		parent::ViewEdit();
 	}
 	...
 	function display(){
           ...
           $pos=$_REQUEST["pos"];
           $sql="SELECT p.name pais, pr.name provincia, c.name ciudad
                 FROM ee_pais p
	...
	<tr>
		...
		$pais = !empty($row['ciudad']) ? $row['ciudad'] : $row['ciudad'];
                $js="'".$row['ciudad']."','".$row['provincia']."','".$row['pais']."','".$pos."'";

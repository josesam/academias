...
class ee_EvaluacionGlobalViewModulos extends ViewEdit {

 	function ee_EvaluacionGlobalViewModulos(){
 		parent::ViewEdit();
 	}
 	...
 	function display(){
          ...
          // echo $ejecucion;
           ...
           $sql="SELECT  modulo,idmodulo id,profesor
	...
	<tr>
        ...
        if(is_array($lista)){
          ...
          foreach ($lista as $key => $row):
        	...
        	$cadena.='<tr>';
			$cadena.='<td><a href="javascript:void(0);" onclick="javascript:selectModulo(\''.$row['modulo']."-".$row['profesor'].'\',\''.$row['id'].'\')">'.$row['modulo'].'</a></td>';
                        $cadena.='<td>'.$row['profesor'].'</td>';
                        $cadena.='</td>';
		$cadena.='</tr>';
            endforeach;
          $cadena.="</tbody>";
